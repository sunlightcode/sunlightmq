	...
	protected void doPost(HttpServletRequest arg0, HttpServletResponse arg1) throws ServletException, IOException {
		...
		if (servletPathToModuleHash.get(servletPath) != null) {
			...
			getServletContext().getRequestDispatcher("/bussinessEntry?module=router&action=rest" + "&" + queryString)
					.forward(arg0, arg1);
		} else if (servletPath.equals("/queue")) {
			getServletContext().getRequestDispatcher("/bussinessEntry?module=queue&action=rest" + "&" + queryString)
					.forward(arg0, arg1);
		} else if (servletPath.equals("/check")) {// 健康检查
			getServletContext().getRequestDispatcher("/bussinessEntry?module=check&action=default" + "&" + queryString)
					.forward(arg0, arg1);
		}
	}
