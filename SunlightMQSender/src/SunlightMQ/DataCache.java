...
public class DataCache {
	private static String[] baseDataTables = { "c_method", "c_priority", "c_acknowledType", "queue", "mqServer" };
	...
	public void loadData() {
		...
		try {
			...
			loadQueue();
			loadMqServer();
			AppLogger.getInstance().infoLog("loadData finish.. mqServerHash is " + mqServerHash.toString());
		} catch (Exception e) {
	...
	public void loadMqServer() throws Exception {
		...
		for (int i = 0; i < methods.size(); ++i) {
			Hashtable<String, String> tmp = methods.get(i);
			if (mqServerHash.get(tmp.get("name")) == null) {
				mqServerHash.put(tmp.get("name"), tmp);
			}
		}
	}
	...
	public Hashtable<String, String> getMqServer(String name) {
		return mqServerHash.get(name) == null ? new Hashtable<String, String>() : mqServerHash.get(name);
	}
}
